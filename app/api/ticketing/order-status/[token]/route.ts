...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  return NextResponse.json({ orderNumber: order.order_number, status: order.status, tickets: (tickets || []).map((ticket) => ({ number: ticket.ticket_number, url: `/tickets/${ticket.verification_token}` })) }, { headers: { "Cache-Control": "no-store" } });
}
