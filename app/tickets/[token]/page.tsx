...
export default async function PublicTicketPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="public-ticket-page"><article className={`digital-ticket status-${ticket.status}`}><header><span>GOOM</span><small>EVENT PRODUCTION</small></header><div className="digital-ticket-event"><p>Admission ticket</p><h1>{event.title}</h1><div><span><CalendarDays />{event.date ? new Intl.DateTimeFormat("en-CA", { dateStyle: "full", timeStyle: "short", timeZone: "America/Toronto" }).format(new Date(event.date)) : "Date to be announced"}</span><span><MapPin />{[event.venue, event.address, event.city].filter(Boolean).join(" · ")}</span></div></div><div className="ticket-qr"><Image src={qr} width={520} height={520} unoptimized alt={`QR code for ticket ${ticket.ticket_number}`} /><strong><TicketCheck />{label}</strong></div><dl><div><dt>Ticket type</dt><dd>{type.name}</dd></div><div><dt>Ticket number</dt><dd>{ticket.ticket_number}</dd></div><div><dt>Attendee</dt><dd>{ticket.attendee_name || "Guest"}</dd></div></dl><footer>Present this individual QR at the entrance. Do not share it publicly.</footer></article><a className="button ticket-pdf-download" href={`/api/tickets/${encodeURIComponent(token)}/pdf`}>Download ticket</a></main>;
}
