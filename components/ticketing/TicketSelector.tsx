...
import { MAX_TICKETS_PER_ORDER, money, publicAvailabilityLabel, publicAvailabilityStatus } from "@/lib/ticketing/core";
import { calculateServiceFeeCents, type ServiceFeeConfig } from "@/lib/ticketing/service-fee";
...
export function TicketSelector({ eventId, ticketTypes, nowIso, serviceFee }: { eventId: string; ticketTypes: TicketTypeRecord[]; nowIso: string; serviceFee: ServiceFeeConfig }) {
  ...
  const fee = calculateServiceFeeCents(subtotal, serviceFee);
  ...
  return <section className="goom-ticket-selector"><div className="ticketing-heading"><p className="section-eyebrow"><span />Official GOOM ticketing</p><h2>Get <em>tickets.</em></h2><p>Select your tickets below. Final availability is confirmed securely when the reservation is created.</p></div>
    <div className="ticket-type-list">{ticketTypes.map((ticket) => { const available = Math.max(0, ticket.quantity_total - ticket.quantity_sold - ticket.quantity_reserved); const availability = publicAvailabilityStatus(ticket); const count = quantities[ticket.id] || 0; const upcoming = Boolean(ticket.sales_start && new Date(ticket.sales_start).getTime() > now); const ended = Boolean(ticket.sales_end && new Date(ticket.sales_end).getTime() <= now); const purchasable = !upcoming && !ended && availability !== "sold_out" && availability !== "hidden" && available > 0; const status = upcoming ? `Tickets go on sale on ${saleDate(ticket.sales_start!)}` : ended ? "Sales ended" : publicAvailabilityLabel(availability); return <article className={`ticket-type-public${purchasable ? "" : " is-unavailable"}`} key={ticket.id}><div><Ticket /><h3>{ticket.name}</h3>{ticket.description && <p>{ticket.description}</p>}<small className={`ticket-availability-badge availability-${availability}`}>{status}</small></div><strong>{money(ticket.price_cents, ticket.currency)} <small>{ticket.currency}</small></strong><div className="ticket-counter"><button type="button" aria-label={`Remove ${ticket.name}`} onClick={() => change(ticket, -1)} disabled={!purchasable || !count}><Minus /></button><span>{count}</span><button type="button" aria-label={`Add ${ticket.name}`} onClick={() => change(ticket, 1)} disabled={!purchasable || available <= count || count >= MAX_TICKETS_PER_ORDER || totalQuantity >= MAX_TICKETS_PER_ORDER}><Plus /></button></div></article>; })}</div>
    <div className="ticket-order-summary"><h3>Order Summary</h3>{selectedTickets.length ? selectedTickets.map((ticket) => <div key={ticket.id}><span>{ticket.name} × {quantities[ticket.id]}</span><strong>{money(ticket.price_cents * quantities[ticket.id], ticket.currency)} <small>{ticket.currency}</small></strong></div>) : <p>No tickets selected yet.</p>}<div><span>Subtotal</span><strong>{money(subtotal, ticketTypes[0]?.currency || "CAD")}</strong></div><div><span>Service fee</span><strong>{money(fee, ticketTypes[0]?.currency || "CAD")}</strong></div><div className="ticket-order-total"><span>Total</span><strong>{money(total, ticketTypes[0]?.currency || "CAD")} <small>{ticketTypes[0]?.currency || "CAD"}</small></strong></div></div>
    ...
    {buyerOpen && <div className="buyer-panel"><div><h3>Buyer details</h3><p>Your tickets will be delivered to this email.</p></div><form action={checkout}><label>Full name<input name="name" autoComplete="name" required /></label><label>Email<input name="email" type="email" autoComplete="email" required /></label><label>Phone<input name="phone" type="tel" inputMode="tel" autoComplete="tel" required minLength={9} placeholder="+1 905 555 1234" /></label>{error && <p className="checkout-error">{error}</p>}<button className="button" disabled={busy}>{busy ? "Reserving…" : "Reserve tickets"}</button><button type="button" className="text-button" onClick={() => setBuyerOpen(false)}>Back</button><p className="secure-note"><ShieldCheck />Your reservation is held for 15 minutes.</p></form></div>}
  </section>;
}
