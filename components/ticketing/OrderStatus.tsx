...
type State = { orderNumber?: string; status?: string; tickets?: Array<{ number: string; url: string }> };
export function OrderStatus({ token }: { token: string }) { const [state, setState] = useState<State>({ status: "pending" });
  ...
  if (state.status === "paid") return <div className="checkout-result success"><span>Payment confirmed</span><h1>Your tickets are ready.</h1><p>{state.orderNumber}</p><div>{state.tickets?.map((ticket) => <Link className="button" href={ticket.url} key={ticket.number}>View {ticket.number}</Link>)}</div></div>;
