-- Record the payment environment on every transaction so maintenance tools
-- can distinguish test data from production data without guessing.

alter table public.orders add column if not exists payment_environment text;

update public.orders
set payment_environment = case
  when payment_provider = 'mock' then 'mock'
  when payment_provider = 'square' then coalesce((select environment from public.payment_connections where provider = 'square'), 'sandbox')
  else 'manual'
end
where payment_environment is null;

alter table public.orders alter column payment_environment set default 'manual';
alter table public.orders alter column payment_environment set not null;
...
alter table public.orders add constraint orders_payment_environment_check
  check (payment_environment in ('mock','sandbox','production','manual'));

comment on column public.orders.payment_environment is
  'Immutable payment origin used to isolate mock/Square Sandbox transactions from production.';

alter table public.payment_webhook_events add column if not exists environment text;
update public.payment_webhook_events
set environment = coalesce((select environment from public.payment_connections where provider = 'square'), 'sandbox')
where environment is null;
alter table public.payment_webhook_events alter column environment set not null;
alter table public.payment_webhook_events drop constraint if exists payment_webhook_events_environment_check;
alter table public.payment_webhook_events add constraint payment_webhook_events_environment_check
  check (environment in ('sandbox','production'));

comment on column public.payment_webhook_events.environment is
  'Square environment that delivered the webhook. Existing rows use the recorded Square connection environment during migration.';
...
declare
  ...
  derived_payment_environment text := 'manual';
...
begin
  ...
  if p_payment_provider = 'mock' then
    derived_payment_environment := 'mock';
  elsif p_payment_provider = 'square' then
    select environment into derived_payment_environment
    from public.payment_connections
    where provider = 'square';
    derived_payment_environment := coalesce(derived_payment_environment, 'sandbox');
  else
  ...
  insert into public.orders(event_id, customer_name, customer_email, customer_phone, payment_provider, payment_environment, reservation_expires_at)
  values (p_event_id, trim(p_customer_name), lower(trim(p_customer_email)), nullif(trim(p_customer_phone), ''), p_payment_provider, derived_payment_environment, expiry)
